...
test_that("... is converging", {
  ...
  iter7 <- em_clust_norm(c_tot, nclust= nclust, itmax= 7)
  iter15 <- em_clust_norm(c_tot, nclust= nclust, itmax= 15)
  
  ## test
  expect_lte(iter1$log_lik, iter7$log_lik)
  expect_lte(iter7$log_lik, iter15$log_lik)
})
