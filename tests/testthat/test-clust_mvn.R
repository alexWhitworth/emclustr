...
context("mvn clustering")

test_that("can catch errors", {
  expect_error(em_clust_mvn(data= letters[1:20], nclust= 1))
  expect_error(em_clust_mvn(data= rexp(100, 1), nclust= 0))
  expect_error(em_clust_mvn(data= rexp(100, 1), nclust= 0.5))
  expect_error(em_clust_mvn(data= letters[1:20], nclust= 1, itmax= 0))
  expect_error(em_clust_mvn(data= letters[1:20], nclust= 1, itmax= 0.5))
  expect_error(em_clust_mvn(data= letters[1:20], nclust= 1, itmax= 10.5))
})
...
test_that("... returns approrpiate output", {
  ...
  nclust <- 3
  test <- em_clust_mvn(c_tot, nclust= nclust, itmax= 5)
...
test_that("... is converging", {
  ...
  iter1 <- em_clust_mvn(c_tot, nclust= nclust, itmax= 1)
  iter3 <- em_clust_mvn(c_tot, nclust= nclust, itmax= 3)
  iter5 <- em_clust_mvn(c_tot, nclust= nclust, itmax= 5)
...
test_that("... does converge before itmax", {
  ...
  c_tot <- rbind(c1,c2,c3); rm(c1,c2,c3)
  itmax <- 10000
  test <- em_clust_mvn(c_tot, nclust= 3, itmax= itmax)
